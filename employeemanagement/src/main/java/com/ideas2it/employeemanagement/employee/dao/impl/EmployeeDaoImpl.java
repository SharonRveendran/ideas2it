...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	private EmployeeManagementLogger employeeManagementLoggerLogger; //= new EmployeeManagementLogger(EmployeeDaoImpl.class);

	public EmployeeDaoImpl(EmployeeManagementLogger employeeManagementLoggerLogger) {
	    this.employeeManagementLoggerLogger = employeeManagementLoggerLogger;
	    employeeManagementLoggerLogger.createLogger(EmployeeDaoImpl.class);
	}
	...
	public void insertEmployee(Employee employee) throws EmployeeManagementException {
		...
		try (Session session = sessionFactory.openSession()) {
			...
			employeeManagementLoggerLogger.logError(getStackTrace(e));
	...
	public Employee getEmployee(int id) throws EmployeeManagementException {
		Employee employee = null;employeeManagementLoggerLogger.logError("-----------------------ppppppppppppppppyyyyyyyyyyy-----------------------");
		try (Session session = sessionFactory.openSession()) {
	...
	public List<Employee> getAllEmployee() throws EmployeeManagementException {
		List<Employee> employees = new ArrayList<Employee>();
		try (Session session = sessionFactory.openSession()) {
			Query query = session.createQuery("from Employee where isDeleted = :isDeleted").setParameter("isDeleted",
					false);
			employees = query.getResultList();
		} catch (HibernateException e) {
			employeeManagementLoggerLogger.logError(getStackTrace(e));
